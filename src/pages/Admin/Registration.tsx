    ...
    return (
      <SaaSCard key={registration.id} style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
          ...
          <div style={metaStyle}>时间：{new Date(registration.createdAt).toLocaleString()}</div>
        </div>
        ...
        {registration.status === "pending" ? (
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => handleApprove(registration.id, true)} style={approveButtonStyle}>通过</button>
            <button onClick={() => handleApprove(registration.id, false)} style={rejectButtonStyle}>驳回</button>
          </div>
        ) : null}
      </SaaSCard>
    );
  ...
  return (
    <div style={pageStyle}>
      <div style={containerStyle}>
        ...
        {!loading && doneList.length > 0 ? <><div style={{ fontSize: 13, fontWeight: 800, color: "#64748B", margin: "18px 2px 10px" }}>最近已处理</div>{doneList.map(renderRegistration)}</> : null}
